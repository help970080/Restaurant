...
app.post('/api/admin/provision', wrap(async (req, res) => {
  ...
  const { nombre = 'Mi Restaurante', adminUser, adminPass, plantilla: plantillaCruda = 'restaurante' } = req.body || {};
...
app.post('/api/pedidos', wrap(async (req, res) => {
  ...
  const ped = await withState((e, c) => {
    ...
    if (tipoServicio === 'domicilio') {
      ...
      if (dir && dir.lat != null) p.reparto.destino = { lat: dir.lat, lng: dir.lng, fuente: dir.fuente || 'gps', verificado: !!dir.verificado };
      return p;
    }
...
app.post('/api/reparto/:folio/entregado', wrap(async (req, res) => {
  ...
  const p = await withState((e, c) => {
    ...
    if (u && M.destinoCreible(u, suc0.coordenadas || ped.reparto.origen)) {
      // Entrega real, lejos del local: ESTE punto sí queda confirmado y a
      // partir de ahora le gana a lo que diga el geocodificador.
      ped.reparto.destino = { lat: u.lat, lng: u.lng, fuente: 'gps', verificado: true };
      M.upsertCliente(e, ped.cliente || {}, { lat: u.lat, lng: u.lng, fuente: 'gps', verificado: true });
    }
    ...
    return await als.run({ row, rol: 'sistema', username: 'geo', sucursalId: null }, async () => {
      ...
      return withState((e) => {
        ...
        if (dif != null && dif > KM_DISCREPANCIA) {
          // Un punto CONFIRMADO por una entrega real le gana a la dirección,
          // pero si dejaron de coincidir es que cambiaron la dirección: se
          // avisa a caja para que decida, en vez de resolverlo a la brava.
          if (guardado.verificado) {
            guardado.dudoso = { lat: punto.lat, lng: punto.lng, km: dif, etiqueta: punto.etiqueta || null };
            return guardado;
          }
          // Cualquier otro punto que contradiga la dirección se descarta: pudo
          // aprenderse de un "entregado" marcado sin salir, y mandar al
          // repartidor a otro lado con toda seguridad es peor que no saber.
          console.log(`[geo] ${folio}: punto guardado a ${dif} km de la dirección escrita — se usa la dirección`);
          const cliMal = (e.clientes || {})[M.llaveTel(c.telefono)];
          if (cliMal && !cliMal.verificado) { cliMal.lat = null; cliMal.lng = null; cliMal.fuente = null; }
          return ponerDireccion();
        }
